...
export class ProductsService {
  ...
  async addImages(productId: string, newImageUrls: string[], userId: string) {
    ...
    return this.prisma.product.update({
      where: { id: productId },
      data: { images: updatedImages, updatedBy: userId },
      include: { category: true, inventory: true },
    });
  }

  async removeImage(productId: string, imageUrl: string, userId: string) {
    const product = await this.findById(productId);
    const currentImages = (product.images as string[]) || [];
